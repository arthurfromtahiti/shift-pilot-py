...
**VÉRIFIÉ_CODE — Quatre articles.** Quatre articles sont définis : `AX-100` (zone A, qty 12, reserved 2 → disponible 10), `BX-220` (zone B, qty 0, reserved 0 → disponible 0), `CX-330` (zone A, qty 45, reserved 5 → disponible 40), `DX-440` (zone C, qty 7, reserved 1 → disponible 6) (`inventory/warehouse.py:4-7`). L'invariant `reserved ≤ qty` est respecté par les quatre articles et couvert par `test_invariant_reserved_ne_depasse_pas_qty_dans_items` (`tests/test_warehouse.py:25-31`) — voir TESTING_AUDIT.md pour la confirmation d'exécution.
...
**VÉRIFIÉ_CODE — Aucune contrainte déclarée au niveau du modèle.** Il n'existe ni assertion sur le schéma, ni validation (`pydantic`, `marshmallow`, `dataclasses`), ni check de contrainte dans le code. Les règles implicites du modèle (SKU unique, `qty ≥ 0`, `reserved ≥ 0`, `reserved ≤ qty`) ne sont pas enforçées au niveau du modèle. La contrainte `available ≥ 0` est cependant garantie par le code de `available_qty()` via `max(0, ...)` (`inventory/warehouse.py:24`) et encodée par deux tests (`tests/test_warehouse.py:14-23`). L'invariant `reserved ≤ qty` est couvert par `test_invariant_reserved_ne_depasse_pas_qty_dans_items` (`tests/test_warehouse.py:25-31`) — voir TESTING_AUDIT.md pour la confirmation d'exécution.
...
- **Cycle rouge→vert abouti et encodé.** `CX-330` illustre un invariant métier traçable : les données seed respectent désormais `reserved ≤ qty` (`reserved=5, qty=45`) et `available_qty()` retourne `40`. Le correctif (données + borne `max(0, ...)`) est couvert par des tests présents (voir TESTING_AUDIT.md pour la confirmation d'exécution). Le modèle de démonstration est cohérent de bout en bout. (`inventory/warehouse.py:4-7`, `tests/test_warehouse.py:14-31`)
