...
- **`inventory/warehouse.py` — seule source de donnée.** Tout repose sur ce fichier de 28 lignes. Si la structure de `ITEMS` change (un champ renommé, un type différent), les deux modules et les tests cassent simultanément. C'est le point de défaillance unique du projet.
