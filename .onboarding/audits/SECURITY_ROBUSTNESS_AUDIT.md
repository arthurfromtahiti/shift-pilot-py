...
- **VÉRIFIÉ_CODE (mitigé) — Garde sur `requested ≤ 0` présente dans `can_fulfil()`.** `can_fulfil()` rejette toute quantité nulle ou négative (`inventory/orders.py:7-8`). Le vecteur d'injection via `requested` négatif (ex. `-6`) est fermé. Reste : un `requested` de type incorrect (ex. `None`, `"3"`) provoquerait une `TypeError` non guidée avant la garde, car la comparaison `requested <= 0` échouerait sur un non-numérique.
