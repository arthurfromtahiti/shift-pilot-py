...
Douze tests total : cinq sur `warehouse.py` (tous verts), sept sur `orders.py` (couvrant `picking_list()`). La qualité des tests existants est correcte pour leur périmètre : ils couvrent les cas nominaux, les cas critiques de disponibilité insuffisante, et le cumul intra-commande (après le correctif SHIAAAAAAAAAAAAAAAAAAAAAAAA-316). Les sept tests de `picking_list()` valident l'exclusion des articles non disponibles, les quantités nulles/négatives, et la règle de cumul intra-commande pour plusieurs lignes du même SKU. La couverture de `orders.py` s'est améliorée — `picking_list()` a une bonne couverture — mais `can_fulfil()` n'a toujours aucun test. Il n'existe ni CI, ni configuration de coverage, ni lint. Les `__pycache__/` ne sont pas ignorés par git (absence de `.gitignore`).

## Constats détaillés

**VÉRIFIÉ_CODE — Suite de tests : deux fichiers, douze méthodes.** `tests/test_warehouse.py` contient une classe `TestWarehouse` avec cinq méthodes (`test_find_by_sku`, `test_items_in_zone`, `test_available_qty_cx330`, `test_available_qty_borne_a_zero_quand_reserved_superieur_a_qty`, `test_invariant_reserved_ne_depasse_pas_qty_dans_items`) ; `tests/test_orders.py` contient une classe `TestPickingList` avec sept méthodes (`test_article_hors_stock_exclu`, `test_cx330_inclus_dans_picking_list`, `test_quantite_nulle_exclue`, `test_quantite_negative_exclue`, `test_plusieurs_lignes_meme_sku_depassement_exclu`, `test_plusieurs_lignes_meme_sku_dans_les_limites`, `test_plusieurs_lignes_meme_sku_allocation_cumulative`) (`tests/test_warehouse.py:6-31`, `tests/test_orders.py:6-46`). La commande de lancement est documentée dans le README (`README.md:10`) ; l'exécution confirme 12 tests lancés, tous passants.
...
**VÉRIFIÉ_CODE — `test_available_qty_cx330` et `test_available_qty_borne_a_zero_quand_reserved_superieur_a_qty` : deux tests verts, validant le correctif CLA-177.** `test_available_qty_cx330` teste que `available_qty(find_by_sku("CX-330"))` retourne `40` (qty=45, reserved=5 après correction) (`tests/test_warehouse.py:14-18`). `test_available_qty_borne_a_zero_quand_reserved_superieur_a_qty` teste qu'un article synthétique avec reserved=15 > qty=10 retourne `0` (`tests/test_warehouse.py:20-23`). Le correctif CLA-177 (`f3d4233`) a appliqué le `max(0, ...)` qui garantit une disponibilité jamais négative. Ces deux tests encodent l'invariant métier (`disponible ≥ 0`) et vérifient que l'implémentation le respecte.

**VÉRIFIÉ_CODE — `inventory/orders.py` : couverture améliorée de `picking_list()`.** `tests/test_orders.py` contient sept tests sur `picking_list()` qui valident l'exclusion des articles indisponibles, les quantités invalides, et le cumul intra-commande (après le correctif SHIAAAAAAAAAAAAAAAAAAAAAAAA-316) (`tests/test_orders.py:7-46`). `can_fulfil()` reste entièrement non testé (`inventory/orders.py:6-12`).
...
- **`can_fulfil()` entièrement non testée ; `picking_list()` couverture améliorée.** `can_fulfil()` reste entièrement non testée — SKU inconnu, quantité zéro, bug de disponibilité négative propagé ne sont vérifiés par aucun test. `picking_list()` dispose maintenant d'une bonne couverture (7 tests) — articles hors stock, disponibles, quantités nulles/négatives, cumul intra-commande — bien que les cas liste vide et ordre de zones multiples ne soient pas spécifiés. (`inventory/orders.py:6-35`)
...
- **Couverture améliorée mais incomplète de `orders.py`** — `picking_list()` dispose maintenant de 7 tests (exclusion sur indisponibilité, quantités nulles/négatives, cumul intra-commande) et sa couverture s'est sensiblement améliorée. `can_fulfil()` est une décision commerciale binaire mais reste entièrement non testé.

## Risques

- **VÉRIFIÉ_CODE — `can_fulfil()` sans test ; couverture améliorée de `picking_list()`.** `tests/test_orders.py` importe `picking_list` et teste sept cas (article hors stock, article disponible, quantités nulles/négatives, cumul intra-commande) ; `can_fulfil()` reste entièrement non testé. HYPOTHÈSE : toute modification de `can_fulfil()` peut introduire une régression non détectée. Le comportement de `picking_list()` sur liste vide, SKU inconnu seul, ou tri de zones multiples est toujours non spécifié.
- **VÉRIFIÉ_CODE — Comportement de `picking_list()` partiellement spécifié.** Sept tests couvrent les cas nominaux et les cas limites du cumul intra-commande (après correctif 316). Le comportement sur liste vide, SKU inconnu seul, ou ordre de zones multiples n'est pas testé. HYPOTHÈSE : en l'absence de couverture complète, un refactoring de `picking_list()` ne peut pas distinguer un comportement intentionnel d'une régression.
