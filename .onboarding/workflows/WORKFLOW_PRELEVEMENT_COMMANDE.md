...
- **Bug hérité sur la disponibilité négative.** La fonction `available_qty()` peut retourner une valeur négative pour `CX-330` (disponible = `-5`), ce qui affecte indirectement `can_fulfil()` et donc `picking_list()`. Pour `CX-330` avec une demande ≥ 1, `can_fulfil()` retourne correctement `False`, mais si `available_qty()` est corrigée sans que le reste du code soit mis à jour, cette dépendance pourrait créer des écarts. (`inventory/warehouse.py:32-33`, `inventory/orders.py:6-10`)
